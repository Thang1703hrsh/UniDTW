...
from utils_fdd import get_optimizer_params, get_optimizer_params_peft, print_args, initialize
from utils_fdd import print_rank, get_rank
from utils_fdd import save_rank
from utils_fdd import all_gather
from utils_fdd import load_parallel, save_parallel
from utils_fdd import get_tokenizer, get_model
...
from distillm.losses import get_fdd_loss  # Import unified FDD loss
...
def get_teacher_model(args, device):
    ...
    if args.model_parallel:
        ...
        customize_model_attention(args, model)
...
# NOTE: get_fdd_loss is now imported from distillm.losses (unified implementation)
# The unified version supports both DTW-based schedule and manual layer mapping
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if teacher_model is not None:
                with torch.no_grad():
                    teacher_model.eval()
                    teacher_outputs = teacher_model(**model_batch, output_attentions=True, 
                ...
                # CRITICAL FIX #1: Use label mask (valid tokens) instead of attention mask
                # attention_mask includes padding tokens → causes NaN in cosine similarity
                fdd_mask = (no_model_batch["label"] != -100).float()

                # Use unified get_fdd_loss from distillm.losses
                # It will use args.teacher_layer_mapping and args.student_layer_mapping
                fdd_loss = get_fdd_loss(
                    teacher_outputs.hidden_states, outputs.hidden_states,
                    fdd_mask,  # ← FIXED: Only compute on valid tokens
                    teacher_model, model,
                    teacher_schedule=None,  # Not used in manual mapping mode
                    student_schedule=None,  # Not used in manual mapping mode
                    args=args  # ← Provides layer_mapping
                )
