...
# ----------------------------------------------------------------------------------------------------------------------
@cuda.jit
def compute_softdtw_cuda(D, gamma, bandwidth, max_i, max_j, n_passes, R, lengths):
    """
    ...
    for p in range(n_passes):
        ...
        if I + J == p and (I < max_i and J < max_j):
            # Skip padded cells beyond the valid length
...
# ----------------------------------------------------------------------------------------------------------------------
@cuda.jit
def compute_softdtw_backward_cuda(D, R, inv_gamma, bandwidth, max_i, max_j, n_passes, E, lengths):
    k = cuda.blockIdx.x
    tid = cuda.threadIdx.x
...
class _SoftDTWCUDA(Function):
    ...
    @staticmethod
    def forward(ctx, D, gamma, bandwidth, lengths=None):
        dev = D.device
        dtype = D.dtype
        ...
        if lengths is None:
            lengths = torch.full((B,), N, device=dev, dtype=torch.int32)
        else:
            lengths = lengths.to(device=dev, dtype=torch.int32)
        ...
        ctx.save_for_backward(D, R.clone(), gamma, bandwidth, lengths)
        # Convert back to original dtype if needed
        result = torch.empty((B,), device=dev, dtype=R.dtype)
        for b in range(B):
            end = int(lengths[b].item())
            result[b] = R[b, end, end]
    ...
    def backward(ctx, grad_output):
        ...
        D, R, gamma, bandwidth, lengths = ctx.saved_tensors
        ...
        for b in range(B):
            ...
            R_local[b, end + 1, :] = -math.inf
            R_local[b, :, end + 1] = -math.inf
            R_local[b, end + 1, end + 1] = R_local[b, end, end]
...
class SoftDTW(torch.nn.Module):
    ...
    def forward_with_cost_matrix(self, C, return_alignment: bool = False, lengths: torch.Tensor | None = None):
        """
        ...
        if not return_alignment:
            return func_dtw(C, self.gamma, self.bandwidth, lengths)

        C = C.clone().detach().requires_grad_(True)
        sdtw_vals = func_dtw(C, self.gamma, self.bandwidth, lengths)
        ...
        if lengths is not None:
            lengths = lengths.to(device=A.device, dtype=torch.long)
